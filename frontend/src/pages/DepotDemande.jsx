...
function DepotDemande() {
  ...
  const loadData = async () => {
    try {
      ...
      const [entreprisesResponse, stagesResponse] = await Promise.all([
        api.get("entreprises/"),
        api.get("stages/"),
      ]);
      ...
      console.error("Erreur chargement dépôt demande :", err);
  ...
  // =====================================================
  // CHANGEMENT DES CHAMPS & FILTRAGE DYNAMIQUE
  // =====================================================
  ...
  const filteredStages = formData.entreprise
    ? stages.filter(
        (stage) =>
          stage.entreprise === Number(formData.entreprise) ||
          stage.entreprise?.id === Number(formData.entreprise)
      )
    : stages;

  // =====================================================
  // CV & LETTRE (GESTION FICHIERS)
  // =====================================================
  ...
  const handleSubmit = async (e) => {
    ...
    if (lettreMode === "texte" && !formData.lettre_motivation_texte.trim()) {
      showNotification(
    ...
    if (lettreMode === "fichier" && !lettreMotivationFile) {
      showNotification(
        "Lettre de motivation obligatoire",
    ...
    try {
      ...
      if (formData.entreprise) {
        data.append("entreprise", Number(formData.entreprise));
      }

      if (formData.stage) {
        data.append("stage", Number(formData.stage));
      }

      data.append("type_demande", formData.type_demande);
      data.append("objet", formData.objet.trim());
      ...
      } else if (lettreMode === "fichier") {
        data.append("lettre_motivation_fichier", lettreMotivationFile);
      }
      ...
      console.error("Erreur dépôt demande :", err);
  ...
  // =====================================================
  // VUES DE CHARGEMENT / ERREUR / DROITS
  // =====================================================
    ...
    <div className="depot-demande-page">
      ...
      <header className="depot-demande-header">
        <div>
          ...
          <p>
            Soumettez votre demande à l'administration et à l'entreprise
            concernée.
          </p>
        </div>
      </header>
      ...
      <section className="depot-demande-card">
        <div className="depot-demande-card-header">
          <div className="depot-demande-main-icon">📤</div>
          <div>
            <h2>Informations de la demande</h2>
            <p>
              Remplissez les informations ci-dessous avant d'envoyer votre
              demande.
            </p>
          </div>
        </div>

        <form className="depot-demande-form" onSubmit={handleSubmit}>
          <div className="depot-demande-grid">
            ...
            <div className="depot-field">
              ...
              >
                ...
                <option value="CONVENTION">Demande de convention</option>
            ...
            <div className="depot-field depot-field-full">
              ...
              <small className="depot-field-help">
                Donnez suffisamment de détails pour faciliter le traitement de
                votre demande.
              </small>
            </div>
                  ...
                  <small className="depot-field-help">
                    ...
                    {lettreMotivationFile && (
                      <>
                        {" "}
                        Fichier sélectionné :{" "}
                        <strong>{lettreMotivationFile.name}</strong>
                      </>
                    )}
                  </small>
                </div>
          ...
          <div className="depot-demande-information">
            <div className="depot-demande-information-icon">🔔</div>
            <div>
          ...
          <div className="depot-demande-actions">
            ...
            <button
              ...
              disabled={saving}
            >
              {saving ? "Envoi en cours..." : "📤 Déposer la demande"}
            </button>
          </div>
        </form>
      </section>
